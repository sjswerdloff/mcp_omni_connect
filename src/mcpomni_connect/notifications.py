            ...
            async for message in sessions[server_name]["session"].incoming_messages:
                ...
                async def refresh_capabilities_task():
                    try:
                        ...
                        logger.info(f"Successfully refreshed capabilities after notification from {server_name}")
                        # Force flush logs
                ...
                try:
                    match message.root:
                        case ResourceUpdatedNotification(params=params):
                            ...
                            task.add_done_callback(lambda t: logger.debug(f"Capability refresh task completed for {server_name}"))
