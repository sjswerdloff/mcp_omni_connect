...
async def _select_model(self, preferences: Optional[ModelPreferences], available_models: List[str]) -> str:
...
async def _get_context(self, include_context: Optional[ContextInclusion], server_name: str) -> str:
...
async def _sampling_callback(self, context: RequestContext["ClientSession", Any], params: CreateMessageRequestParams) -> CreateMessageResult | ErrorData:
    ...
    try:
        ...
        model = await self._select_model(params.model_preferences, available_models)
        
        # Get context if requested
        server_name = context.client_id if hasattr(context, 'client_id') else "default"
        additional_context = await self._get_context(params.include_context, server_name)
        ...
        if additional_context:
            messages.append({"role": "system", "content": f"Context: {additional_context}"})
        messages.extend([{"role": msg.role, "content": msg.content.text} for msg in params.messages])
