...
#                 }
#                 result = await mock_client._connect_to_single_server(server_info, "server1")

#                 assert result == "test_server connected succesfully"
#                 mock_exit_stack.assert_called_once()
#                 mock_stack.enter_async_context.assert_has_calls(
#                     [
#                         call(stdio_client(ANY)),
#                         call(ClientSession(ANY, ANY, read_timeout_seconds=timedelta(seconds=300))),
#                     ]
#                 )
...
#                 }
#                 result = await mock_client._connect_to_single_server(server_info, "server2")
...
#         with patch("builtins.open", mock_open(read_data=json.dumps(MOCK_SERVER_CONFIG))):
#             result = await mock_client.add_servers(config_file)

#         assert "new_server connected successfully" in result
